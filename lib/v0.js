 ...
 *
 * @param {Array<string>} labels - Issue label names
 * @returns {number} Complexity level 1-5 (defaults to 3 if not found)
 */
export function extractComplexityFromLabels(labels = []) {
    for (const label of labels) {
        const match = label.match(/complexity[:\s-]*(\d)/i) || label.match(/^L(\d)$/i);
        if (match) {
            const level = parseInt(match[1], 10);
            if (level >= 1 && level <= 5) return level;
        }
    }
    return 3; // Default to Pro tier
}
 ...
 * @param {string} [params.projectId] - Optional V0 project ID
 * @param {'sync'|'async'|'experimental_stream'} [params.responseMode='async'] - Response mode
 * @param {'mini'|'pro'|'max'} [params.model] - V0 model tier
 * @param {boolean} [params.thinking] - Enable thinking mode (Max tier only)
 * @returns {Promise<{ chatId: string, demoUrl: string, files: Array, chatUrl: string }>}
 */
export async function createV0Session({ prompt, system, projectId, responseMode = 'async', model, thinking }) {
    const options = {
        message: prompt,
        config: {
            responseMode,
    ...
    if (model) {
        ...
        if (model === 'max' && thinking) {
            options.config.thinking = true;
        }
    }
