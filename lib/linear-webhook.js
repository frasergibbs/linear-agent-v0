...
async function handleAgentSessionCreated(data) {
    ...
    try {
        ...
        await linearClient.createAgentActivity({
            ...
            content: {
                type: 'thought',
                body: 'Analyzing UI requirements and detecting repository...',
            },
        });
        ...
        try {
            ...
            await linearClient.createAgentActivity({
                agentSessionId: sessionId,
                content: { type: 'error', body: `Failed to create V0 session: ${error.message}` },
            });
        } catch (e) {
...
async function handleAgentSessionPrompted(data) {
    ...
    try {
        ...
        await linearClient.createAgentActivity({
            agentSessionId: sessionId,
            content: { type: 'thought', body: 'Processing your feedback...' },
        });
        ...
        if (guidance?.signals) {
            ...
            if (repoSignal?.value) {
                ...
                await linearClient.createAgentActivity({
                    agentSessionId: sessionId,
                    content: { type: 'message', body: `🎨 Initialized with ${repoSignal.value}\n\n[View in V0](${v0Session.chatUrl})` },
                });
        ...
        if (!session) {
            await linearClient.createAgentActivity({
                agentSessionId: sessionId,
                content: { type: 'error', body: 'No active V0 session found. Please delegate the issue again.' },
            });
        ...
        if (isDeployCommand) {
            await linearClient.createAgentActivity({
                agentSessionId: sessionId,
                content: { type: 'action', body: 'Deploying to Vercel...' },
            });
        ...
        await linearClient.createAgentActivity({
            agentSessionId: sessionId,
            content: { type: 'action', body: 'Applying your feedback...' },
        });
        ...
        try {
            ...
            await linearClient.createAgentActivity({
                agentSessionId: sessionId,
                content: { type: 'error', body: `Error: ${error.message}` },
            });
        } catch (e) {
            console.error('Failed to emit error activity:', e);
        }
