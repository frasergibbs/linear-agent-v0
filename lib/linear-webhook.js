...
import { generateUIGenerationPlan, createAgentPlan, updateAgentPlan, markStepCompleted, markStepCurrent } from './agent-plans.js';
...
async function handleAgentSessionCreated(data) {
    ...
    try {
        ...
        console.log('✅ Emitted Agent Plan');
        ...
        // 5. Update plan - repository import step
        if (repoDetection.repoUrl) {
            planSteps = markStepCurrent(planSteps, 1); // "Import repository context"
            await updateAgentPlan({ sessionId, steps: planSteps });
        }

        // 6. Create or find V0 project
        ...
        if (repoDetection.repoUrl) {
            ...
            // Update plan - repo imported, generating
            planSteps = markStepCompleted(planSteps, 1);
            planSteps = markStepCurrent(planSteps, 2);
            await updateAgentPlan({ sessionId, steps: planSteps });
        ...
        // 8. Set externalUrls on agent session
        await linearClient.updateAgentSession({
            id: sessionId,
            externalUrls: [
                {
                    label: 'V0 Chat',
                    url: v0Session.chatUrl,
                },
            ],
        });

        console.log('✅ Set externalUrls');

        // 9. Update plan - generating complete
        const genIndex = repoDetection.repoUrl ? 2 : 1;
        planSteps = markStepCompleted(planSteps, genIndex);
        planSteps = markStepCurrent(planSteps, genIndex + 1); // Review
        await updateAgentPlan({ sessionId, steps: planSteps });

        // 10. Emit tool activity with V0 chat URL
        await linearClient.createAgentActivity({
            agentSessionId: sessionId,
            content: {
                type: 'tool',
        ...
        // 11. Emit message with links
        await linearClient.createAgentActivity({
            agentSessionId: sessionId,
            content: {
                type: 'message',
                text: `🎨 UI generation complete!\n\n[View in V0](${v0Session.chatUrl})\n\n@mention me to iterate or deploy to Vercel.`,
            },
        });
        ...
        if (guidance?.signals) {
            ...
            if (repoSignal?.value) {
                ...
                await linearClient.updateAgentSession({
                    id: sessionId,
                    externalUrls: [
                        { label: 'V0 Chat', url: v0Session.chatUrl },
                    ],
                });
